...
+ (void) deleteAllOrderLines {
    NSString *query = [NSString stringWithFormat:@"delete from %@", TBL_GMORDERLINE];
    
    sqlite3_exec([DBManager sharedManager], [query UTF8String], NULL, NULL, NULL);
}

+ (BOOL) deleteWhereLine:(NSInteger)PKID {
    NSString *query = [NSString stringWithFormat:@"delete from %@ where (%@ = %ld)", TBL_GMORDERLINE, FLD_PKID, (long)PKID];
...
+ (BOOL) deleteWhere:(NSInteger)uid {
    NSString *query = [NSString stringWithFormat:@"delete from %@ where %@ = %ld", TBL_GMORDERLINE, FLD_ORDERNUM, (long)uid];
...
+ (id) fetch:(sqlite3_stmt *)dbStmt {
    ...
    GMOrderLine * model = [[GMOrderLine alloc] init];
...
+ (NSMutableArray*) getAllOrderLines {
    ...
    NSString * query = [NSString stringWithFormat:@"select * from %@", TBL_GMORDERLINE];
    ...
    id data = [GMOrderLine fetch:stmt];
    while (data) {
        [list addObject:data];
        data = [GMOrderLine fetch:stmt];
    }
...
+ (NSMutableArray*) getOrderLinesWhere:(NSInteger)OrderNum {
    ...
    NSString * query = [NSString stringWithFormat:@"select * from %@ where %@ = %ld", TBL_GMORDERLINE, FLD_ORDERNUM, (long)OrderNum];
