...
@interface EditOrderScreenVC ()

@end

@implementation EditOrderScreenVC
...
- (void)viewDidLoad {
    ...
    btnTxtToFill.titleLabel.text = [NSString stringWithFormat:@"You have picked %ld %@s", (long)orderLineInfo.Quantity, orderLineInfo.ItemNo];
    cellSelected = nil;
    numbersArray = [[NSMutableArray alloc] init];
    for (int i = 0; i < 100; i++) {
        [numbersArray addObject:[NSString stringWithFormat:@"%d", i]];
    }
    btnBadItemCount.title = [NSString stringWithFormat:@"Defective Item Count"];
    
    NSMutableArray * unsortedNames = [OrderDetail getOrderDetailsWhereOrder:orderLineInfo];
    
    self.allTableData = [unsortedNames sortedArrayUsingComparator:^NSComparisonResult(id a, id b) {
        NSString *first = [(OrderDetail*)a Name];
        NSString *second = [(OrderDetail*)b Name];
...
- (void)count {
    ...
    btnTxtToFill.titleLabel.text = [NSString stringWithFormat:@"You have picked %ld %@s", (long)numItemsOrdered, orderLineInfo.ItemNo];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSArray* arrayForLetter = (NSArray*)[filteredTableData objectForKey:letter];
    OrderDetail* tmpOrderDetail = (OrderDetail*)[arrayForLetter objectAtIndex:indexPath.row];
    if ([orderLineInfo.ItemNo isEqual:@"DR1002"]) {
...
- (IBAction)btnSave:(id)sender {
    ...
    tmpOrderLine.ItemNo = orderLineInfo.ItemNo;
    tmpOrderLine.Nameset = orderLineInfo.Nameset;
    NSInteger tmpNum = 0;
    for (OrderDetail* orderDetail in allTableData)
        tmpNum += orderDetail.QtyOrdered;
    tmpOrderLine.Quantity = tmpNum;
    tmpOrderLine.BadItems = orderLineInfo.BadItems;
    tmpOrderLine.ProgName = orderLineInfo.ProgName;
    
    [OrderLine updateOrderLine:tmpOrderLine];
    ...
    for (OrderDetail* orderDetail in allTableData) {
        ...
        [OrderDetail updateOrderDetail:orderDetail];
    }
...
- (IBAction)btnDelete:(id)sender {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle: [NSString stringWithFormat:@"Delete %@?", orderLineInfo.ItemNo]
                                                    message:[NSString stringWithFormat:@"This will remove all %@s from this order.", orderLineInfo.ItemNo]
                                                   delegate:self
                                          cancelButtonTitle:@"Cancel"
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1) {
        if (deleteItem) {
            deleteItem = NO;
            NSMutableArray *countArray = [OrderLine getOrderLinesWhere:orderHeaderInfo.OrderNum];
            if (countArray.count > 1) {
                [OrderLine deleteWhere:orderLineInfo.OrderNum Item:orderLineInfo.ItemNo];
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Only Item"
                                                                message:@"This is the only item in this order, please delete the entire order if you wish to remove"
                                                               delegate:self
                                                      cancelButtonTitle:@"OK"
                                                      otherButtonTitles:nil];
                [alert setAlertViewStyle:UIAlertViewStyleDefault];
