...
#pragma mark - Find Duration

+ (void)showImage:(UIImage *)image status:(NSString *)string {
    [[SVProgressHUD sharedView] showImage:image status:string duration:1.3];
}
...
- (void)setStatus:(NSString *)string {
    ...
    if(string) {
        ...
        UIFont *fontText = [UIFont systemFontOfSize:16];
        CGRect stringSize = [string boundingRectWithSize:maximumLabelSize
                                                 options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading)
                                              attributes:@{NSFontAttributeName:fontText}
                                                 context:nil];
        // code taken out to update to iOS7
...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.fadeOutTimer = [NSTimer scheduledTimerWithTimeInterval:2.0 target:self selector:@selector(dismiss) userInfo:nil repeats:NO];
    });
}
