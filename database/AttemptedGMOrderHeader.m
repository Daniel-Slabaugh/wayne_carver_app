...
@implementation AttemptedGMOrderHeader
@synthesize OrderNum;
@synthesize UserID;
@synthesize PONum;
...
+ (void) deleteAllOrderHeaders {
    NSString *query = [NSString stringWithFormat:@"delete from %@", TBL_ATTEMPTEDGMORDERHEADER];
    
    sqlite3_exec([DBManager sharedManager], [query UTF8String], NULL, NULL, NULL);
}

+ (BOOL) deleteWhere:(NSInteger)uid {
    NSString *query = [NSString stringWithFormat:@"delete from %@ where %@ = %ld", TBL_ATTEMPTEDGMORDERHEADER, FLD_ORDERNUM, (long)uid];
...
+ (id) fetch:(sqlite3_stmt *)dbStmt {
    ...
    AttemptedGMOrderHeader * model = [[AttemptedGMOrderHeader alloc] init];
    ...
    model.DateAttempted = [NSString stringWithFormat:@"%s", (char *)sqlite3_column_text(dbStmt, 23)];
...
+ (NSMutableArray*) getAllOrderHeaders {
    ...
    NSString * query = [NSString stringWithFormat:@"select * from %@", TBL_ATTEMPTEDGMORDERHEADER];
    ...
    id data = [AttemptedGMOrderHeader fetch:stmt];
    while (data) {
        [list addObject:data];
        data = [AttemptedGMOrderHeader fetch:stmt];
    }
...
+ (AttemptedGMOrderHeader*) getLastOrderHeaderEntered {
    NSMutableArray *list = [[NSMutableArray alloc] init];
    
    NSString * query = [NSString stringWithFormat:@"select MAX(%@) from %@", FLD_ORDERNUM, TBL_ATTEMPTEDGMORDERHEADER];
    ...
    if (sqlite3_prepare_v2([DBManager sharedManager], sql, (int)strlen(sql), &stmt, NULL) != SQLITE_OK) {
        NSLog(@"didn't work");
        AttemptedGMOrderHeader *badOrdHdr = [list objectAtIndex:0];
        return badOrdHdr;
    }
    ...
    AttemptedGMOrderHeader *tmpOrdHdr = [list objectAtIndex:0];
    
    return tmpOrdHdr;
}

+ (AttemptedGMOrderHeader*) getOrderHeaderWhereOrderNum:(NSInteger)OrderNum {
    NSMutableArray *list = [[NSMutableArray alloc] init];
    
    NSString * query = [NSString stringWithFormat:@"select * from %@ where %@ = %ld", TBL_ATTEMPTEDGMORDERHEADER, FLD_ORDERNUM, (long)OrderNum];
    ...
    {
        NSLog(@"ERROR");
        AttemptedGMOrderHeader *tmpOrder = [list objectAtIndex:0];
        return tmpOrder;
    }
    id data = [AttemptedGMOrderHeader fetch:stmt];
    while (data) {
    ...
    if (list.count == 0) {
        ...
        return [[AttemptedGMOrderHeader alloc] init];
    }
    AttemptedGMOrderHeader *tmpOrder = [list objectAtIndex:0];
    return tmpOrder;
}

+ (NSInteger) updateOrderHeader:(AttemptedGMOrderHeader*)data {
...
+ (NSInteger) insertInto:(AttemptedGMOrderHeader*)data {
    NSCharacterSet *charactersToRemove = [NSCharacterSet characterSetWithCharactersInString:@"'"];
    if ((NSNull *)data.StoreID != [NSNull null]) {
