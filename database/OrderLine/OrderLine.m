...
+ (void) deleteAllOrderLines {
    NSString *query = [NSString stringWithFormat:@"delete from %@", TBL_ORDERLINE];
    
    sqlite3_exec([DBManager sharedManager], [query UTF8String], NULL, NULL, NULL);
}

+ (BOOL) deleteWhere:(NSInteger)ordNum Item:(NSString*)item {
    NSString *query = [NSString stringWithFormat:@"delete from %@ where (%@ = %ld and %@ = '%@')", TBL_ORDERLINE, FLD_ORDERNUM, (long)ordNum, FLD_LITEMNO, item];
...
+ (BOOL) deleteWhere:(NSInteger)uid {
    NSString *query = [NSString stringWithFormat:@"delete from %@ where %@ = %ld", TBL_ORDERLINE, FLD_ORDERNUM, (long)uid];
...
+ (id) fetch:(sqlite3_stmt *)dbStmt {
    ...
    OrderLine * model = [[OrderLine alloc] init];
...
+ (NSMutableArray*) getAllOrderLines {
    ...
    NSString * query = [NSString stringWithFormat:@"select * from %@", TBL_ORDERLINE];
    ...
    id data = [OrderLine fetch:stmt];
    while (data) {
        [list addObject:data];
        data = [OrderLine fetch:stmt];
    }
...
+ (NSMutableArray*) getOrderLinesWhere:(NSInteger)OrderNum {
    ...
    NSString * query = [NSString stringWithFormat:@"select * from %@ where %@ = %ld", TBL_ORDERLINE, FLD_ORDERNUM, (long)OrderNum];
